...
const category = process.env.F1_CATEGORY ?? 'f1';
...
const raceId = process.env.F1_RACE_ID
  ? template === 'weekend-schedule'
    ? String(process.env.F1_RACE_ID)
    : Number(process.env.F1_RACE_ID)
  : undefined;
...
const manualAdjustments = process.env.F1_MANUAL_ADJUSTMENTS ?? '';
const voiceoverEnabled = !['false', '0', 'off', 'no'].includes(
  String(process.env.F1_VOICEOVER_ENABLED ?? 'true').toLowerCase()
);
